...
Avec le département Qualité des services en ligne, nous avons organisé deux démonstrations, dans une finalité pédagogique, avec pour objectif de comprendre comment un non-voyant utilise son ordinateur et son smartphone pour naviguer sur un site web.
...
Pour commencer, il faut comprendre que les aveugles travaillent sur un ordinateur à travers un logiciel que l'on nomme un **lecteur d'écran**. Le logiciel restitue les informations visibles à l'écran à travers une synthèse vocale et/ou un lecteur braille. Le lecteur d'écran, retranscrit également la nature de l'élément en cours de consultation, comme un lien, un bouton ou une case à cocher. Le lecteur d'écran lit donc dans le code source et intéprête les types de composants. Par exemple un titre visible uniquement par l'augmentation de la taille de ses caractères ne sera pas accessibile facilement. Alors que s'il est borné par les balise `<h2>` et `</h2>`il sera repéré rapidement et facilitera la compréhension du contenu.

> Pour Jamshid, une page accessible est donc une page qui est simplement et correctement codée.

Autre point important, pour naviguer sur un ordinateur, un aveugle n'utilise pas sa souris. La navigation passe alors exclusivement par des commandes du clavier, comme par exemple les flèches ou la tabulation. Dans notre cas, sur la plateforme data.gouv.fr, nous avons pu comprendre qu'il existe plusieurs stratégies de navigation et de recherche de contenus. 
...
Par exemple la recherche plein texte sur l'ensemble du contenu de la page est un mode de navigation exhaustif mais aussi long et fastidieux. Alors que le mode de navigation par titres permet de découvrir tous les titres et de bien souvent  de comprendre la hiérarchisation de l'information. Il existe aussi d'autres stratégies de navigation comme la recherche par liens, par boutons ou par champs de saisie de la page. Ce dernier permet bien souvent d'obtenir un accès au moteur de recherche du site.

Sur la plateforme data.gouv.fr nous avons ensuite cherché un jeu de donnée. Ce jeu de donnée était un fichier Excel que nous avons pu télécharger et analyser. A la fin de cette première démonstration, un intérêt a été formalisé pour comprendre : Comment un non-voyant utilise et navigue en ligne avec son smartphone ?


### 2. Démonstration sur mobile - Ameli, Tchap... 

Lors de la seconde démonstration, Jamshid était accompagné de Fernando Pinto da Silva également expert en accessibilité numérique et non-voyant. Ils ont tous deux utilisés un iPhone et le lecteur d'écran VoiceOver intégré au système d'exploitation MacOs (TalkBack est le lecteur d'écran Android). La démonstration a permis de comprendre les nombreuses gestuelles de navigation et les problématiques que les aveugles peuvent rencontrer avec leur smartphone. 

Comme sur un ordinateur, la navigation avec un lecteur d'écran utilise une voix synthétique. La voix décrit ainsi tout ce que l'utilisateur touche. Contrairement au fonctionnement sans lecteur d'écran qui agit directement lorsque l'utilisateur touche son écran.
...
Evidemment la navigation "à l'aveugle (comme dirait l'autre)" permet difficilement d'arriver à ses fins. C'est pourquoi il existe entre 50 et 60 gestuelles de navigation avec le lecteur d'écran sur mobile. Nous avons ainsi découvert quelques gestuelles, comme par exemple :
...
- Le mouvement circulaire avec deux doigts sur l’écran, comme si l'on souhaitait remonter une vieille horloge, afin de sélectionner une option


#### Application Ameli

Il est également possible de connecter un lecteur en braille nommé une **plage braille** (en bluetooth) afin de naviguer sur son mobile. Fernando a ainsi pu ouvrir l'application Ameli à travers sa plage braille, et en désactivant la voie. Ce dispositif est utile par exemple pour les personnes sourdes-aveugles.

Ensuite dans le cadre de la démonstration nous avons pu ouvrir l'[application Ameli](https://apps.apple.com/fr/app/ameli-lassurance-maladie/id620447173) afin de comprendre ce qui était accessible et ce qui ne l'était pas. Nous avons pu ainsi observer un bouton sans label, nous questionnant sur l'utilité de celui-ci, un champ de texte décrit comme étant à saisir alors qui ne l'était pas et un bouton cité comme "Dashboard menu BTN white".

#### Application Tchap...

Jamshid nous a ensuite présenté l'[application Tchap](https://www.tchap.gouv.fr/). Dans la navigation, nous avons observé quelques éléments mal décrits ou même non définis, laissant planer rapidement un doute sur les actions à réaliser. Le développement web est un travail rigoureux.

Ensuite nous avons pu échanger des messages sur l'application Tchap, nous permettant de comprendre comment utiliser le clavier virtuel du smartphone.

Une seconde application, nommé [Scanner](https://apps.apple.com/us/app/scanner/id1291962681https://apps.apple.com/us/app/scanner/id1291962681), nous a été présenté par Jamshid. Celle-ci permet de prendre en photo une page contenant du texte. Le texte est ensuite lu par la synthèse vocale, et permet par exemple de lire l'addition d'un restaurant ou un courrier administratif.

Jamshid nous a également présenté une troisième application nommé [BlindSquare](https://apps.apple.com/fr/app/blindsquare/id500557255). Cette application a été conçue pour les aveugles afin de connaître tout ce qui se situe dans votre environnement (restaurant, parc, musées, boutiques...). Couplé à une application de navigation (Maps ou Plan), ces applications permettent tous les jours, à Jamshid, de se déplacer.

#### Voir l'inaccessible

Pour conclure, l'observation de l'utilisateur, afin de comprendre son expérience particulière et ses usages, est riche d'enseignement ; pour nous concepteur, ces démonstrations ont permis d'appréhender les difficultés et les bonnes pratiques d'accessibilité, mais aussi d'utilisabilité, de nos interfaces. Nous vérifions trop souvent nos sites dans notre propre contexte, il est indispensable de se décentrer.

L'accessibilité numérique semble être un domaine chargé de normes et de règles techniques. En réalité, elle définit uniquement ce qui doit être produit par les concepteurs et les développeurs web selon un standard. Cela demande de la rigueur et cela est utile car il existe quantité de contextes utilisateur que nous ne maîtrisons pas.
